...
def cw_delay(
    ...
    fplus, fcross, cosMu = utils_jnp.create_gw_antenna_pattern(pos, gwtheta, gwphi)
...
def bwm_delay(
    ...
    if antenna_pattern_fn is None:
        apc = utils_jnp.create_gw_antenna_pattern(pos, gwtheta, gwphi)
    else:
...
def compute_eccentric_residuals(
    ...
    if check:
        # check that frequency is not evolving significantly over obs. time
        y = utils_jnp.solve_coupled_ecc_solution(
    ...
    gammadot = utils_jnp.get_gammadot(F, mc, q, e0)
    ...
    ##### earth term #####
    splus, scross = utils_jnp.calculate_splus_scross(
        nmax=nharm,
        mc=mc,
        dl=dist,
        h0=h0,
    ...
    if psrTerm:
        ...
        if np.any(y):
            ...
            gammadotp = utils_jnp.get_gammadot(Fp, mc, q, ep)
            ...
            nharm = min(nharm, 100)
            splusp, scrossp = utils_jnp.calculate_splus_scross(
                nmax=nharm,
                mc=mc,
                dl=dist,
                h0=h0,
